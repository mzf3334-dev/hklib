...
### Generating the Report on GitHub Actions
...
The history report workflow also publishes the report to GitHub Pages, giving you a direct URL to view the result — no download needed:
...
- When the report workflow finishes, an email is sent to `EMAIL_RECEIVER` with a clickable link to the published page (uses the same Gmail settings as the renewal notifications).
